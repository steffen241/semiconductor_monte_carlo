...
kim_1e16 = pl.zeros((19,2))

kim_1e16[:,0] = kim_ana_tmp[:,0]*1e5
kim_1e16[:,1] = kim_ana_tmp[:,1]*1e5

f = open('InAlAs/inalas_kim/300K_1e17.csv')
data = csv.reader(f)
kim_ana_tmp = pl.array(list(data))
kim_ana_tmp = pl.float128(kim_ana_tmp)
kim_1e17 = pl.zeros((18,2))

kim_1e17[:,0] = kim_ana_tmp[:,0]*1e5
kim_1e17[:,1] = kim_ana_tmp[:,1]*1e5

f = open('InAlAs/inalas_kim/300K_4e17.csv')
data = csv.reader(f)
kim_ana_tmp = pl.array(list(data))
kim_ana_tmp = pl.float128(kim_ana_tmp)
kim_4e17 = pl.zeros((21,2))

kim_4e17[:,0] = kim_ana_tmp[:,0]*1e5
kim_4e17[:,1] = kim_ana_tmp[:,1]*1e5

f = open('InAlAs/inalas_kim/300K_1e18.csv')
data = csv.reader(f)
kim_ana_tmp = pl.array(list(data))
kim_ana_tmp = pl.float128(kim_ana_tmp)
kim_1e18 = pl.zeros((17,2))

kim_1e18[:,0] = kim_ana_tmp[:,0]*1e5
kim_1e18[:,1] = kim_ana_tmp[:,1]*1e5
...
vel.plot(kim_1e16[:,0],kim_1e16[:,1],'^-',linewidth=2,label='Kim 1e16 $cm^{-3}$',color=(0,cl[1],0,1))
vel.plot(kim_1e17[:,0],kim_1e17[:,1],'^-',linewidth=2,label='Kim 1e17 $cm^{-3}$',color=(0,cl[3],0,1))
vel.plot(kim_1e18[:,0],kim_1e18[:,1],'^-',linewidth=2,label='Kim 1e18 $cm^{-3}$',color=(0,cl[4],0,1))
...
vel.set_xlim([5e4, 0.25e7])
vel.set_ylim([0, 2e5])
vel.set_xlabel('Electric Field (V/m)',fontsize=18)
vel.set_ylabel('Drift Velocity (m/s)',fontsize=18)
vel.legend(loc='center right',bbox_to_anchor=(1.13, 0.5),fontsize=14)
fig.show()
#fig.savefig('InAlAs_drift_vel_doping.pdf',dpi=300)
...
fig.show()
#fig.savefig('InAlAs_drift_mu_doping.pdf',dpi=300)