module config
    ...
    integer,parameter                          :: num_carriers = 400000!1000000
...
!                                            & 2000.0_s]*1e4_s/1e9_s
    ...
    real(kind=s),parameter          :: t_sim                = 6.0_s !80.0_s !40.0_s
    real(kind=s),parameter          :: t_step               = 0.0001_s !0.005_s!1.0
    ...
    real(kind=s)                            :: dev_x                = 110.0_s           ! Devfice length: x-direction
    ...
    integer,dimension(:),allocatable        :: num_nodes
    ! Mesh configuration
    real(kind=s),dimension(4,3)             :: m                    = reshape((/0.0_s,80.0_s,0.0_s,dev_y, &
                                                                            &  80.0_s,90.0_s,0.0_s,dev_y, &
                                                                            &  90.0_s,110.0_s,0.0_s,dev_y/),(/4,3/))
...
!                                                                        & INGAAS,INALAS/)!(/INALAS, INGAAS, INALAS/) (/INGAAS,INGAAS/)!
    ...
    real(kind=s),dimension(3)                 :: donor_density = (/0.0_s,1e19_s,0.0_s/)/1e21_s
    real(kind=s),dimension(:,:),allocatable   :: donor
    ...
    integer,dimension(2)                     :: save_x         = (/70,111/)
    integer,dimension(2)                     :: save_y         = (/1,401/)
    integer                                  :: save_t         = 100! 200
    ...
    subroutine init_mesh_2()
        ...
        contact_id(s_ohm-0:e_ohm+1,1) = 1
        ! top
        ...
        !contact_open(s_ohm:e_ohm,num_nodes(2)) = 1
        contact_id(s_ohm-0:e_ohm+1,num_nodes(2)) = 2
        ...
        contact_type(num_nodes(1),1:(num_nodes(2))) = INSULATOR
         ...
         contact_pot(num_nodes(1),1:num_nodes(2)) = 0.0_s !-0.738_s
